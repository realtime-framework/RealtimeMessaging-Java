...
public class Ortc {
	...
	public static boolean saveAuthentication(String url, boolean isCluster,
		...
		try {
			...
			isAuthenticated = Authentication.saveAuthentication(
					...
					privateKey, permissionsMap,new Proxy());
		} catch (Exception e) {
	 ...
	 *
	 * if (!Ortc.saveAuthentication(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;,
	 * 		true, &quot;SessionId&quot;, false, &quot;APPKEY&quot;, 1800, &quot;PVTKEY&quot;, new Proxy(&quot;my.proxy.net&quot;, 1234), permissions)) {
	 * 	throw new Exception(&quot;Was not possible to authenticate&quot;);
	 * }
	 * </pre>
	 *
	...
	public static boolean saveAuthentication(String url, boolean isCluster,
											 String authenticationToken, boolean authenticationTokenIsPrivate,
											 String applicationKey, int timeToLive, String privateKey,
											 Map<String, ChannelPermissions> permissions, Proxy proxy) throws IOException,
			InvalidBalancerServerException,
			OrtcAuthenticationNotAuthorizedException {
		String connectionUrl = url;
		if (isCluster) {
	 ...
	 * 
	 * if (!Ortc.saveAuthentication(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;,
	 * 		true, &quot;SessionId&quot;, false, &quot;APPKEY&quot;, 1800, &quot;PVTKEY&quot;, permissions,  new Proxy(&quot;my.proxy.net&quot;, 1234))) {
	 * 	throw new Exception(&quot;Was not possible to authenticate&quot;);
	 * }
	 * </pre>
	 * 
	...
	public static boolean saveAuthentication(String url, boolean isCluster,
			String authenticationToken, boolean authenticationTokenIsPrivate,
			String applicationKey, int timeToLive, String privateKey,
			HashMap<String, LinkedList<ChannelPermissions>> permissions, Proxy proxy)
			throws IOException, InvalidBalancerServerException,
			OrtcAuthenticationNotAuthorizedException {
		String connectionUrl = url;
		if (isCluster) {
	...
	public static void saveAuthentication(String url, boolean isCluster,
		...
		saveAuthentication(url, isCluster, authenticationToken, authenticationTokenIsPrivate, applicationKey, timeToLive, privateKey, permissionsMap, new Proxy(), onCompleted);
	}
	...
	public static void saveAuthentication(String url, boolean isCluster,
										  final String authenticationToken, final boolean authenticationTokenIsPrivate,
										  final String applicationKey, final int timeToLive, final String privateKey,
										  final Map<String, ChannelPermissions> permissions, final Proxy proxy,
										  final OnRestWebserviceResponse onCompleted) throws IOException,
			InvalidBalancerServerException,
			OrtcAuthenticationNotAuthorizedException {
		...
		saveAuthentication(url, isCluster, authenticationToken, authenticationTokenIsPrivate, applicationKey, timeToLive, privateKey, permissionsMap, proxy, onCompleted);
	}
	...
	public static void saveAuthentication(String url, boolean isCluster,
		...
		if (isCluster) {
			Balancer.getServerFromBalancerAsync(url, applicationKey,proxy, new OnRestWebserviceResponse() {
				@Override
				public void run(Exception error, String response) {
					if(error != null){
						onCompleted.run(error, null);
					}else{
						saveAuthenticationAsync(response, authenticationToken, authenticationTokenIsPrivate, applicationKey, timeToLive, privateKey, permissions, proxy, onCompleted);
					}
				}
			});
		}else{
			saveAuthenticationAsync(connectionUrl, authenticationToken, authenticationTokenIsPrivate, applicationKey, timeToLive, privateKey, permissions, proxy, onCompleted);
		}		
	}
	...
	public static void saveAuthentication(String url, boolean isCluster,
										  final String authenticationToken, final boolean authenticationTokenIsPrivate,
										  final String applicationKey, final int timeToLive, final String privateKey,
										  final HashMap<String, LinkedList<ChannelPermissions>> permissions,final Proxy proxy,
										  final OnRestWebserviceResponse onCompleted) throws IOException,
			InvalidBalancerServerException,
			OrtcAuthenticationNotAuthorizedException {
		String connectionUrl = url;
		if (isCluster) {
			Balancer.getServerFromBalancerAsync(url, applicationKey,new Proxy(), new OnRestWebserviceResponse() {
				@Override
				public void run(Exception error, String response) {
					if(error != null){
						onCompleted.run(error, null);
	...
	private static void saveAuthenticationAsync(String url, 
			...
			String applicationKey, int timeToLive, String privateKey,
			HashMap<String, LinkedList<ChannelPermissions>> permissions,
			Proxy proxy, OnRestWebserviceResponse onCompleted){
		try {
			URL authenticationUrl = new URL(String.format("%s/authenticate",
					url));
	...
	public static void presence(String url, Boolean isCluster,
		...
		Presence.getPresence(url, isCluster, applicationKey,
				authenticationToken, channel, new Proxy(), callback);
	}
	/**
	...
	public static void enablePresence(String url, Boolean isCluster,
		...
		Presence.enablePresence(url, isCluster, applicationKey, privateKey,
				channel, metadata, new Proxy(),callback);
	}
	/**
