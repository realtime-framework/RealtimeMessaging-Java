...
public class Ortc {
   ...
   * permissions.put(&quot;channel1&quot;, ChannelPermissions.Write);
   * 
   * if (!Ortc.saveAuthentication(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;, true, &quot;SessionId&quot;, false, &quot;APPKEY&quot;, 1800, &quot;PVTKEY&quot;,
   *     permissions)) {
   *   throw new Exception(&quot;Was not possible to authenticate&quot;);
   * }
   * </pre>
  ...
  public static boolean saveAuthentication(String url, boolean isCluster, String authenticationToken,
      ...
      Map<String, ChannelPermissions> permissions) throws IOException, InvalidBalancerServerException,
      OrtcAuthenticationNotAuthorizedException {
    String connectionUrl = url;
    if (isCluster) {
      connectionUrl = Balancer.getServerFromBalancer(url,applicationKey, new Proxy());
    }
    ...
    try {
      ...
      isAuthenticated = Authentication.saveAuthentication(authenticationUrl,authenticationToken, authenticationTokenIsPrivate, applicationKey, timeToLive, privateKey, permissionsMap, new Proxy());
    } catch (Exception e) {
  ...
  /**
   * Saves the authentication token channels permissions in the ORTC server (using proxy connection).
   * 
   * <pre>
   * HashMap&lt;String, ChannelPermissions&gt; permissions = new HashMap&lt;String, ChannelPermissions&gt;();
   * permissions.put(&quot;channel1:*&quot;, ChannelPermissions.Write);
  ...
  public static boolean saveAuthentication(String url, boolean isCluster, String authenticationToken,
      boolean authenticationTokenIsPrivate, String applicationKey, int timeToLive, String privateKey,
      HashMap<String, LinkedList<ChannelPermissions>> permissions) throws IOException, InvalidBalancerServerException,
      OrtcAuthenticationNotAuthorizedException {
    String connectionUrl = url;
    if (isCluster) {
      connectionUrl = Balancer.getServerFromBalancer(url, applicationKey, new Proxy());
    ...
    try {
      ...
      isAuthenticated = Authentication.saveAuthentication(authenticationUrl, authenticationToken, authenticationTokenIsPrivate,
          applicationKey, timeToLive, privateKey, permissions, new Proxy());
    } catch (Exception e) {
      throw new OrtcAuthenticationNotAuthorizedException(e.getMessage());
    }
   ...
   * 
   * <pre>
   * Ortc.presence(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;, true, &quot;APPLICATION_KEY&quot;, &quot;AUTHENTICATION_TOKEN&quot;, &quot;CHANNEL&quot;,
   *     new onPresence() {
   * 
   ...
   */
  public static void presence(String url, Boolean isCluster, String applicationKey, String authenticationToken, String channel, OnPresence callback) {
    Presence.getPresence(url, isCluster, applicationKey, authenticationToken, channel, new Proxy(), callback);
  }
  /**
   * Gets the subscriptions in the specified channel and if active the first 100 unique metadata (using proxy connection).
   * 
   * <pre>
   ...
   * 
   * <pre>
   * Ortc.enablePresence(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;, true, &quot;APPLICATION_KEY&quot;, &quot;PRIVATE_KEY&quot;, &quot;CHANNEL&quot;, true,
  ...
  public static void enablePresence(String url, Boolean isCluster, String applicationKey, String privateKey, String channel,
    ...
    Presence.enablePresence(url, isCluster, applicationKey, privateKey, channel, metadata, new Proxy(), callback);
  }
  /**
   ...
   * 
   * <pre>
   * Ortc.disablePresence(&quot;http://ortc-developers.realtime.co/server/2.1/&quot;, true, &quot;APPLICATION_KEY&quot;, &quot;PRIVATE_KEY&quot;, &quot;CHANNEL&quot;,
   ...
   *          Callback with error and result.
   */
  public static void disablePresence(String url, Boolean isCluster, String applicationKey, String privateKey, String channel, OnDisablePresence callback) {
	  Presence.disablePresence(url, isCluster, applicationKey, privateKey, channel, new Proxy(), callback);
  }
  /**
   * Disables presence for the specified channel (using proxy connection).
   * 
   * <pre>
